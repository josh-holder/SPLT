...
if __name__ == "__main__":
	log_name = "sequences/log_randomParanoid.txt"

	#when continuation=True, picks up from where a previous sim left off
	continuation = False

	continuation_name = "sequences/random_paranoid_continuation.pkl"
	if continuation:
	...
	while 1:
		exploring_from = 2000
		random_start_length = 50
		random_start_gb = core.Board()
		for i in range(random_start_length):
			moveOptions=random_start_gb.getMoveOptions()
			if len(moveOptions) > 0:
				nextMove=random.choice(moveOptions)
			else:
				exploring_from = 0 #restart the random start if you somehow managed to lose before move 50
				break

			remainingDistance = random.random()*sum(random_start_gb.weights)

			move = -1

			for i,weight in enumerate(random_start_gb.weights):
				remainingDistance -= weight
				if remainingDistance < 0:
					move = i
					break

			core.makeMove(random_start_gb,move)

		random_start_sequence = random_start_gb.splitRecord
		while exploring_from > 250:
			...
			#don't add all almost as good options because the problem becomes intractible
			if len(game_board.splitRecord)>600: add_every_blank_good_options = 3
			elif len(game_board.splitRecord)>875: add_every_blank_good_options = 2
		...
		print(f"$$$ Finished exploring this random start (exploring_from now {exploring_from}). Reseeding random start! $$$")