...
def findWeights(gameBoard,weightverbose=0):
	...
	if weightverbose: print(" ")

	#Here, we check that in spltting the best split we're not accidentally creating new clusters
	#i.e. new blocks fall into a position where they create a cluster, something which the algorithm
	#cannot predict as is
	checked_for_unintended = False
	splits_checked_for_unintented = set()
	while not checked_for_unintended:
		best_split = max(range(len(weights)), key=weights.__getitem__)
		testgb = copy.deepcopy(gameBoard)
		if testgb.box[best_split].splitPossible(testgb.splitAction):
			basic_core.makeMove(testgb,best_split)
			#We make a couple of approximations here:
			# 1.we just check the most recently created box and see if it now has points and is larger than 1x1.
			# 	in the case that we meant to split the most recently created box and create a big point block,
			# 	we're now double counting the negative weight.
			# 2.For weight penalty calculations, we assume that the cluster created was of 4 blocks. This 
			#	may undercount the negative weight in some scenarios, but it will likely still be enough
			# 	to disqualify the algorithm from taking this action.
			...
			if ((most_recent_box.points != 0) and (most_recent_box.width != 1 or most_recent_box.height != 1) and best_split not in splits_checked_for_unintented):
				...
				splits_checked_for_unintented.add(best_split)
			else:
				if weightverbose: print("Finished checking for unintended consequences.")
				checked_for_unintended = True
		else:
			if weightverbose: print("Finished checking for unintended consequences.")
			checked_for_unintended = True
...
if __name__ == '__main__':
	gb = core.Board()
	core.makeMove(gb,0)
	findWeights(gb,weightverbose=1)
	# core.makeMove(gb,0)
	# core.makeMove(gb,0)
	# core.makeMove(gb,0)
	# core.makeMove(gb,1)
