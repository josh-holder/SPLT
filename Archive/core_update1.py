...
import weighting1 as weighting
...
##########################################
	...
	def __init__(self,width=None,height=None,splitRecord=None):
		...
		#for each box, self.clusters contains a list of the indices of other boxes that would be involved in a cluster when the box was split
		self.horClusters=[[]] 
		self.verClusters =[[]]

		#these variables track the columns and rows that were effected by explosions and splits the previous turn
		self.explosionEffectColumns = []
		self.splitEffectColumns = []
		self.splitEffectRows = []

		for i in range(self.width):
			self.explosionEffectColumns.append(1)
			self.splitEffectColumns.append(1)
		
		for i in range(self.height):
			self.splitEffectRows.append(1)
		...
		#Intializes the number of horizontal and vertical splits on the board
		self.hor_splits = (self.height-1)
		self.ver_splits = (self.width-1)
...
def makeMove(gameBoard,chosenBox):
	...
	print("--------------------------------------")
	print("Move number "+str(len(gameBoard.splitRecord)))
	print("Splitting box "+str(chosenBox))
	print("--------------------------------------")

	global verbose

	#initializes list of boolean values which determines if any changes occured 
	explosionEffectColumns = []
	splitEffectColumns = []
	splitEffectRows = []
	for i in range(gameBoard.width):
		explosionEffectColumns.append(0)
		splitEffectColumns.append(0)
	
	for i in range(gameBoard.height):
		splitEffectRows.append(0)
	
	chosenBox_box = gameBoard.box[chosenBox]
	for i in range(chosenBox_box.x-1,chosenBox_box.x+chosenBox_box.width+1):
		if 0 <= i < gameBoard.width:
			splitEffectColumns[i] = 1
	for i in range(chosenBox_box.y-1,chosenBox_box.y+chosenBox_box.height+1):
		if 0 <= i < gameBoard.height:
			splitEffectRows[i] = 1
	...
	if gameBoard.split(gameBoard.box[chosenBox])==0:
		print("Problem trying to split box {0}, aborting".format(chosenBox));exit()
	...
	#We've already scanned and found which blocks would be clusters, so we just read them off from the appropriate clusters variable
	pointsToAdd = len(gameBoard.splitRecord)+1
	
	print("Adding points:")
	if gameBoard.splitAction == HORIZONTAL:
		for index in gameBoard.verClusters[chosenBox]:
			print("Adding points to box "+str(index))
			try: gameBoard.box[index].points = pointsToAdd
			except: 
				raise Exception("Failed at index "+str(index))
			
	else:
		for index in gameBoard.horClusters[chosenBox]:
			print("Adding points to box "+str(index))
			try: gameBoard.box[index].points = pointsToAdd
			except: raise Exception("Failed at index "+str(index))
	...
	print("Removing point blocks. hor and ver Clusters before removal:")
	print(gameBoard.horClusters)
	print(gameBoard.verClusters)
	blocksDestroyed = 0

	for boxIndex, box in enumerate(gameBoard.box):
		if box.points<0: #box should be destroyed
			blocksDestroyed += 1
			#update explosionEffectColumns with a 1 if something exploded within one column of the column
			for index in range(box.x-1,box.x+box.width+1):
				if 0 <= index < gameBoard.width:
					explosionEffectColumns[index] = 1

			#remove their clusters from ver and horizontal clusters
			gameBoard.verClusters.pop(boxIndex)
			gameBoard.horClusters.pop(boxIndex)
	...
	for cluster in gameBoard.verClusters:
		if len(cluster) != 0:
			for index in cluster:
				index = index - blocksDestroyed
	
	for cluster in gameBoard.horClusters:
		if len(cluster) != 0:
			for index in cluster:
				index = index - blocksDestroyed
			print("edited cluster:")
			print("cluster")

	print("Point blocks removed. hor and ver Clusters after removal:")
	print(gameBoard.horClusters)
	print(gameBoard.verClusters)	
	...
	gameBoard.hor_splits = 0
	gameBoard.ver_splits = 0
	gameBoard.boxesInRow = defaultdict(list)
	for boxIndex,box in enumerate(gameBoard.box):
		if box.points == 0:
			gameBoard.hor_splits += box.height-1
			gameBoard.ver_splits += box.width-1
	...
	print("Columns affected by explosions this turn:")
	print(explosionEffectColumns)
	print("Columns affected by explosions last turn:")
	print(gameBoard.explosionEffectColumns)

	print("Columns affected by splits this turn:")
	print(splitEffectColumns)
	print("Columns affected by splits last turn:")
	print(gameBoard.splitEffectColumns)

	print("Rows affected by splits this turn:")
	print(splitEffectRows)
	print("Rows affected by splits last turn:")
	print(gameBoard.splitEffectRows)

	print("Now, find the new clusters for each box in the board as necessary.")

	if gameBoard.splitAction == HORIZONTAL:
		#print("(Clusters will be caused by horizontal splits)")
		gameBoard.horClusters = weighting.findCluster(gameBoard,explosionEffectColumns,splitEffectColumns,splitEffectRows)
	else:
		#print("(Clusters will be caused by vertical splits)")
		gameBoard.verClusters = weighting.findCluster(gameBoard,explosionEffectColumns,splitEffectColumns,splitEffectRows)
...
if __name__ == "__main__":
	...
	makeMove(gameBoard,3)
	makeMove(gameBoard,4)
	makeMove(gameBoard,5)
	makeMove(gameBoard,7)
	makeMove(gameBoard,7)
	makeMove(gameBoard,2)
	makeMove(gameBoard,2)
	makeMove(gameBoard,0)
	makeMove(gameBoard,11)
	makeMove(gameBoard,13)
	makeMove(gameBoard,9)
	makeMove(gameBoard,3)
	makeMove(gameBoard,3)
	makeMove(gameBoard,0)
	makeMove(gameBoard,13)
	...
	makeMove(gameBoard,1)
	makeMove(gameBoard,0)
	makeMove(gameBoard,18)
	makeMove(gameBoard,11)
	makeMove(gameBoard,17)
	"""
	"""
	makeMove(gameBoard,5)
	makeMove(gameBoard,2)
