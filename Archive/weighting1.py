...
def findCluster(gameBoard, explosionEffectColumns, splitEffectColumns, splitEffectRows):
	...
	if gameBoard.splitAction == HORIZONTAL:
		oldClusters = gameBoard.horClusters
	else:
		oldClusters = gameBoard.verClusters

	saved = 0
	#iterates through every box in the board and splits them
	for boxToBeCheckedIndex, boxToBeChecked in enumerate(gameBoard.box):
		if boxToBeChecked.points == 0 and boxToBeChecked.splitPossible(gameBoard.splitAction):
			if clusterverbose: 
				print(" ")
				print("Box " + str(boxToBeCheckedIndex)+" is not a point block and can be split in this way, so we determine if we need to update the list of clusters.")

			#determines if the box needs an update this turn based on changes made this turn
			needUpdate = 0
			for col in range(boxToBeChecked.x,boxToBeChecked.x+boxToBeChecked.width):
				if explosionEffectColumns[col]: needUpdate = 1
				
				if splitEffectColumns[col]:
					for row in range(boxToBeChecked.y,boxToBeChecked.y+boxToBeChecked.height):
						if splitEffectRows[row]: needUpdate = 1
			
			#determines if the box needs an update based on changes that happened last turn
			neededUpdate = 0
			for col in range(boxToBeChecked.x,boxToBeChecked.x+boxToBeChecked.width):
				if gameBoard.explosionEffectColumns[col]: neededUpdate = 1
				
				if gameBoard.splitEffectColumns[col]:
					for row in range(boxToBeChecked.y,boxToBeChecked.y+boxToBeChecked.height):
						if gameBoard.splitEffectRows[row]: neededUpdate = 1

			if clusterverbose: 
				print("x: "+str(boxToBeChecked.x)+" y: "+str(boxToBeChecked.y)+ " width: "+str(boxToBeChecked.width)+ " height: "+str(boxToBeChecked.height))
				print("Last turn, did it change: "+ str(neededUpdate))
				print("This turn, did it change: "+ str(needUpdate))

			#if the box needed an update either this turn or last turn, update the box
			if  neededUpdate or needUpdate or (len(oldClusters)<boxToBeCheckedIndex+1):
				
				newClusters = set()

				newgameBoard = copy.deepcopy(gameBoard)
				newgameBoard.box = copy.deepcopy(gameBoard.box)
				newgameBoard.splitAction = copy.copy(gameBoard.splitAction)
				...
				if clusterverbose:
					print("Clusters for box "+str(boxToBeCheckedIndex)+" must be updated, so we split it.")
					print("New box from split: points, x, y = "+str(lastCreatedBox.points)+" "+str(lastCreatedBox.x)+" "+str(lastCreatedBox.y))
				...
				for clusterBoxIndex, clusterBox in enumerate(newgameBoard.box):
					...
					if clusterBox == lastCreatedBox:
						...
						if len(clusterMembers) == 4:
							...
							if clusterverbose: 
								print("Box "+str(clusterBoxIndex)+" is in the upper left corner of a cluster when box " + str(boxToBeCheckedIndex)+" was split. Cluster is:")
								print(newClusters)
				
				if clusterverbose: 
					print("After update, final list of boxes in box "+str(boxToBeCheckedIndex)+"'s cluster is:")
					print(newClusters)
			...
			#otherwise, keep the old.
				...
				saved += 1
				if clusterverbose: print("DIDNT need to update: keeping the old clusters, but update the highest index box.")
				if gameBoard.splitAction == HORIZONTAL:
					if clusterverbose: 
						print("Old horizontal clusters:")
						print(gameBoard.horClusters[boxToBeCheckedIndex])
					if len(gameBoard.horClusters[boxToBeCheckedIndex]) > 0:
						gameBoard.horClusters[boxToBeCheckedIndex].remove(max(gameBoard.horClusters[boxToBeCheckedIndex]))
						gameBoard.horClusters[boxToBeCheckedIndex].add(len(gameBoard.box))
					if clusterverbose:
						print("New horizontal clusters:")
						print(gameBoard.horClusters[boxToBeCheckedIndex])
					clusters.append(gameBoard.horClusters[boxToBeCheckedIndex])
				else:
					if clusterverbose: 
						print("Old vertical clusters:")
						print(gameBoard.verClusters[boxToBeCheckedIndex])
					if len(gameBoard.verClusters[boxToBeCheckedIndex]) > 0:
						gameBoard.verClusters[boxToBeCheckedIndex].remove(max(gameBoard.verClusters[boxToBeCheckedIndex]))
						gameBoard.verClusters[boxToBeCheckedIndex].add(len(gameBoard.box))
					if clusterverbose:
						print("New vertical clusters:")
						print(gameBoard.verClusters[boxToBeCheckedIndex])
					clusters.append(gameBoard.verClusters[boxToBeCheckedIndex])	
		else:
			"""
			if clusterverbose:
				if boxToBeChecked.points != 0:
					print(" ")
					print("Box "+str(boxToBeCheckedIndex)+" is a point block, so we don't check clusters.")
				elif not boxToBeChecked.splitPossible(gameBoard.splitAction):
					print(" ")
					print("Box "+str(boxToBeCheckedIndex)+" can't be split in this way, so we don't check clusters.")
			"""
			clusters.append(set())
	

	#Reset the gameBoard variables to be the gameBoard variables from the current turn
	if clusterverbose: 
		print("Done with updating clusters for all boxes: Setting last turns affected columns to be this turns affected columns.")
		if gameBoard.splitAction == HORIZONTAL:
			print("Final list of horizontal clusters for the gameBoard:")
			print(clusters)
		else:
			print("Final list of vertical clusters for the gameBoard:")
			print(clusters)

	gameBoard.explosionEffectColumns = explosionEffectColumns
	gameBoard.splitEffectRows = splitEffectRows
	gameBoard.splitEffectColumns = splitEffectColumns

	if timingInfo: 
		end = time.time()
		print("Time to find clusters for turn "+str(len(gameBoard.splitRecord))+": "+str(end-start))
		print("Saved calculating clusters for "+str(saved)+" boxes.")
	return clusters
...
def findWeights(gameBoard):
	...
	if gameBoard.splitAction == HORIZONTAL:
		clusters = gameBoard.horClusters
	else:
		clusters = gameBoard.verClusters
	...
	for boxToBeWeightedIndex, boxToBeWeighted in enumerate(gameBoard.box):
		...
		if boxToBeWeighted.points == 0 and boxToBeWeighted.splitPossible(gameBoard.splitAction):
			...
			try:
				createPoints = boxToBeWeightedIndex in clusters[boxToBeWeightedIndex]
			except:
				print("index is out of range of clusters.")
				print("index: "+str(boxToBeWeightedIndex))
				print(clusters)
			...
			if createPoints:
				...
				maxHeight = 0
				for boxIndex in clusters[boxToBeWeightedIndex]:
			...
			if createPoints:
				...
				clusterSquares = len(clusters[boxToBeWeightedIndex])/2*splitsAvailable
			...
			if len(clusters[boxToBeWeightedIndex]) == 6:
				weightToAdd = -10
			else:
				weightToAdd = 0
	...
	if minWeight <=0:
		minWeight = -minWeight
		weights = [x+minWeight+.1 for x in weights]
