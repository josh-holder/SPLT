...
def findWeights(gameBoard, weightCoef):
	"""
	...
	#weightCoef, a list of the coefficients of the equations needed to calculate the weights
	...
	for boxToBeWeightedIndex, boxToBeWeighted in enumerate(gameBoard.box):
		...
		if boxToBeWeighted.points == 0 and boxToBeWeighted.splitPossible(gameBoard.splitAction):
			...
			if gameBoard.splitAction == HORIZONTAL:
				...
				if not createPoints:
					...
					if splitImbalance <= 0:
						...
						weightToAdd = (i1)*verticalSplitsToAdd #Eyeballed to provide -15 weight when adding the max
						...
						weightToAdd = weightToAdd*(i2*abs(splitImbalance)/total_splits)
						...
						weightToAdd = -i3*percent**2+i4*percent
			...
			#calculate imbalance weight for vertical splits
				...
				if not createPoints:
					...
					if splitImbalance >= 0:
						...
						weightToAdd = (i1)*horizontalSplitsToAdd #Eyeballed to provide -15 weight when adding the 7 new horizontal splits
						#Scales based off how urgent the imbalance is relative to how many total splits are left. 2x weight when the imbalance is 50% of the total splits left
						weightToAdd = weightToAdd*(i2*abs(splitImbalance)/total_splits)
			...
			if newaspectRatio - aspectRatio > 1:
				...
				weightToAdd = ar1*(newaspectRatio - aspectRatio)**2+ar2*(newaspectRatio - aspectRatio)
				if weightverbose: 
			...
			elif aspectRatio - newaspectRatio > 1:
				weightToAdd = -ar1*(newaspectRatio - aspectRatio)**2-ar2*(newaspectRatio - aspectRatio)
				if weightverbose: 
					print("Aspect ratio is getting better, going from "+str(aspectRatio)+" to "+str(newaspectRatio))
					print("We add a penalty weight of "+str(weightToAdd))
			...
			if splitsUntilFall[boxToBeWeightedIndex] != 0:
				...
				if createPoints:
					...
					if difference >= 2**extraSplitsBelow:
						...
						weightToAdd = (pb1/(1+pb2*math.exp(pb3*turns)))+pb4
				...
				#otherwise, if splitting the box won't cause a cluster, we prioritize splitting boxes in this row, but less aggressively
					...
					if 2 < splitsUntilFall[boxToBeWeightedIndex] <= 18:
						...
						weightToAdd = pb4*splitsUntilFall[boxToBeWeightedIndex]**2+pb5*splitsUntilFall[boxToBeWeightedIndex]-pb6
					else:
						weightToAdd = 0
			...
			if createPoints:
				...
				weightToAdd = h1*(maxHeight/gameBoard.height)**2
				...
				weightToAdd = h2*((boxToBeWeighted.y+boxToBeWeighted.height)/gameBoard.height)**2
			...
			if createPoints:
				...
				if clusterSquares > 4:
					weightToAdd = pbs1*(clusterSquares/total_splits)*(len(gameBoard.splitRecord)/pbs2)
				else:
					weightToAdd=0
